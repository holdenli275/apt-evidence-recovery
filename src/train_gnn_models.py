...
import psutil
import time
from pygod.detector import OCRGCN
from pygod.metric import eval_roc_auc
import numpy as np

from database_config import OCRAPT_DATASETS, OCRAPT_HOSTS
...
parser.add_argument('--detector', type=str, default="OCRGCN", choices=["OCRGCN"])
...
parser.add_argument('--start-run', type=int, default=0, help='first run index (for appending runs to an existing summary)')
...
assert args.dataset in OCRAPT_DATASETS
assert args.host in OCRAPT_HOSTS
dataset_numofClasses = str(args.n_classes)

root_path = args.root_path

# Data dir: prefer the persistent '_pyg' offline cache when present, so the
# script never reads from the transient extracted directory (which other
# tasks may delete/re-extract concurrently).
data_root = root_path + args.exp_name + '_pyg' if os.path.isdir(root_path + args.exp_name + '_pyg') else root_path + args.exp_name
...
def pyGod(run,seed):
    ...
    def load_features(node_type):
        feat_path = data_root + "/features/" + node_type + "/node-features.pt"
    ...
    def inintailize_model(contamination,num_relations,batch_size):
        ...
        return OCRGCN(num_relations=num_relations, hid_dim=hidden_channels,
                      num_layers=args.num_layers, epoch=args.epochs,
                      batch_size=batch_size, dropout=args.dropout, lr=args.lr,
                      contamination=contamination, save_emb=args.save_emb,
                      beta=args.beta, warmup=args.warmup, gpu=gpu,
    ...
    def log_raised_alarms(homo_data, y_pred, score, y_prob,target_node=None,alerts_2hop=None,run=0):
        ...
        if target_node:
            file_path = data_root + "/mapping/" + target_node + "_entidx2name.csv"
            ...
            for subject_node in subject_nodes:
                file_path = data_root + "/mapping/" + subject_node + "_entidx2name.csv"
...
if __name__ == '__main__':
    ...
    for run in range(args.start_run, args.start_run + args.runs):
        # deterministic seed per run (run 0 keeps the original seed 360);
        # makes --start-run appends reproducible across separate invocations
        seed = 360 + run
        print("******************************************")
        ...
        if torch.cuda.is_available():
            print(f"[run {run}] GPU reserved before empty: {torch.cuda.memory_reserved()/1e9:.2f} GB, allocated: {torch.cuda.memory_allocated()/1e9:.2f} GB")
            torch.cuda.empty_cache()
            print(f"[run {run}] GPU reserved after empty: {torch.cuda.memory_reserved()/1e9:.2f} GB")
    ...
    ensure_dir(out_path)
    # --start-run: append new per-run rows to any existing summary and recompute avg/std
    per_run_new = anomaly_results_df[anomaly_results_df['run'].astype(str).str.isdigit()].copy()
    if os.path.isfile(out_path):
        old_df = pd.read_csv(out_path)
        old_per_run = old_df[old_df['run'].astype(str).str.isdigit()].copy()
        combined = pd.concat([old_per_run, per_run_new], ignore_index=True)
        print(f"[merge] existing {len(old_per_run)} per-run rows + new {len(per_run_new)} -> {len(combined)}")
    else:
        combined = per_run_new
    combined_numeric = combined.drop(columns=['run'])
    mean_all = combined_numeric.mean(0)
    mean_all['run'] = 'avg'
    std_all = combined_numeric.std(0)
    std_all['run'] = 'std'
    combined = pd.concat([combined, pd.DataFrame([mean_all, std_all])], ignore_index=True)
    combined.to_csv(out_path, index=None)
    print(f"[merge] summary written: {out_path}")
