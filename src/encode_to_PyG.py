...
from database_config import OCRAPT_DATASETS, OCRAPT_HOSTS, order_x_features
...
parser.add_argument('--use-tgn', action="store_true", default=False)
parser.add_argument('--tgn-embeddings', type=str, default=None)
...
def concat_tgn_embeddings(mapped_x_list_df, args):
    # Append TGN temporal embeddings (keyed by node_uuid) to per-type features.
    # node-features.pt rows follow entidx2name.csv order (node_id ascending),
    # and pd.merge preserves the left (mapping) order, so alignment is by row.
    emb_path = args.tgn_embeddings
    if emb_path is None:
        emb_path = os.path.join(args.root_path, "features",
                                args.exp_name + "_tgn_embeddings.pt")
    print("Loading TGN embeddings from:", emb_path)
    tgn_emb = torch.load(emb_path, map_location="cpu", weights_only=False)
    vecs = np.vstack([np.asarray(tgn_emb[str(u)], dtype=np.float32)
                      for u in mapped_x_list_df["node_uuid"]])
    for i in range(vecs.shape[1]):
        mapped_x_list_df["tgn_" + str(i)] = vecs[:, i]
    print("Concatenated TGN embeddings, per-node feature dim now:",
          len(mapped_x_list_df.columns) - 2)
    return mapped_x_list_df
...
def splitbyStratifiedNodeTypes(g_tsv_df,this_type_node_idx,entites_dic,labels_rel_df,label_node):
    ...
    if args.training_valid:
        ...
        # A small evaluation split may contain only one node of a type.
        if len(eval_df) < 2:
            test_df = eval_df.copy()
            valid_df = eval_df.copy()
        elif len(eval_df["o_idx"].unique()) == 1:
...
if __name__ == '__main__':
    ...
    for label_node in label_nodes_names:
        ...
        if args.use_tgn:
            mapped_x_list_df = concat_tgn_embeddings(mapped_x_list_df, args)
