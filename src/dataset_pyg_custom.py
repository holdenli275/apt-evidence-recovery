...
class PygNodePropPredDataset_custom(InMemoryDataset):
    def __init__(self, name, root = '../dataset/',numofClasses=349, transform=None, pre_transform=None, meta_dict = None, split_by='time'):
        ...
        if meta_dict is None:
            ...
            # Offline cache: '<root><name>_pyg' is a sibling of the extracted
            # '<root><name>' dir; reuse it when present so repeated runs do not
            # delete/re-extract the zip (avoids same-host directory conflicts).
            self._zip_path = self.dir_name + '.zip'
            self._cache_dir = self.original_root + name + '_pyg'
            if osp.isdir(self._cache_dir) and osp.isfile(osp.join(self._cache_dir, 'processed', 'geometric_data_processed.pt')):
                self.dir_name = self._cache_dir
        ...
        self.meta_info = {'url': getattr(self, '_zip_path', self.dir_name + '.zip')}
        f = zipfile.ZipFile(getattr(self, '_zip_path', self.dir_name + '.zip'), 'r')
    ...
    def download(self):
        ...
        if str(url).startswith("http")==False:
            path = url
            # persistent cache: extract once into <root><name>_pyg/, reuse forever
            if getattr(self, '_cache_dir', None) and osp.isdir(self._cache_dir):
                print('Using cached dataset dir:', self._cache_dir)
            else:
