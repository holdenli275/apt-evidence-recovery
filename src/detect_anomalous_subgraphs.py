...
from database_config import OCRAPT_DATASETS, OCRAPT_HOSTS, get_attack_time_range
import pytz
...
parser.add_argument('--score-node-types', type=str, default=None, help='Only sum Prediction_probability over these node types (comma-separated, e.g. PROCESS,FILE); default: all types')
...
def clear_and_insert_anomalies(anomalies_nodes):
    ...
    # NOTE(fix): the released loop was `range(int(len/1000)+1)`, which emits an extra
    # EMPTY chunk whenever len(anomalies) is an exact multiple of 1000.  The empty slice
    # stringifies to 'Series([], )' and is then posted as Turtle -> GraphDB 400
    # (Lexical error, `after prefix "Series"`).  Iterate over non-empty chunks only.
    for start_bulk in range(0, len(turtle_anomalies), 1000):
        end_bulk = min(start_bulk + 1000, len(turtle_anomalies))
...
def subgraph_anomaly_score(subgraph):
    ...
    correlated_anomalies_nodes = anomalies_nodes[anomalies_nodes["node_uuid"].isin(all_correlated_nodes)]
    if args.score_node_types is not None:
        type_aliases = {
        ...
        allowed_bases = set()
        for _t in args.score_node_types.split(','):
            _t = _t.strip().lower()
            allowed_bases.add(_t)
            allowed_bases.update(type_aliases.get(_t, []))
        _seg = correlated_anomalies_nodes['node_type'].str.split('_').str[-1].str.lower()
        _full = correlated_anomalies_nodes['node_type'].str.lower()
        correlated_anomalies_nodes = correlated_anomalies_nodes[_seg.isin(allowed_bases) | _full.isin(allowed_bases)]
...
def get_edges_mapping_of_full_graph():
    ...
    data_root = dataset.root  # persistent '_pyg' cache dir when present
...
if __name__ == '__main__':
    ...
    for run in range(args.runs):
        ...
        # deterministic per-run seed (matches train_gnn_models.py: seed = 360 + run);
        # run0 still uses 360, so released/shuffled results are unchanged.
        seed = 360 + run + 1
    print("\n************************************************************")
