...
class OCRGCNBase(nn.Module):
    ...
    **kwargs
    ...
    def loss_func_val(self, emb, label,threshold, fig_title=None, visualize=False):
        ...
        if sum(label) > 0:
            validation_auc = roc_auc_score(label.cpu(), validation_score.detach().cpu())
        ...
        validation_f1_score = f1_score(y_true=label.cpu(), y_pred=validation_pred.cpu(), zero_division=0)
