    ...
    def forward(self, x: Union[OptTensor, Tuple[OptTensor, Tensor]],
        ...
        if self.num_blocks is not None:  # Block-diagonal-decomposition =====
            ...
            if (torch_geometric.typing.WITH_PYG_LIB and self.num_bases is None
                ...
                # reproduction on limited GPU memory: chunked message passing
                # equivalent to the pyg-lib fast path (x_j @ weight[rel],
                # then scatter-mean over target nodes), but bounded memory.
                from torch.utils.checkpoint import checkpoint
                out = torch.zeros(x_r.size(0), self.out_channels,
                                  device=x_r.device)
                deg = torch.zeros(x_r.size(0), device=x_r.device)
                chunk_size = 500000
                for rel in range(self.num_relations):
                    idx = (edge_type == rel).nonzero(as_tuple=False).view(-1)
                    w = self.weight[rel]
                    for s in range(0, idx.size(0), chunk_size):
                        e = idx[s:s + chunk_size]

                        def _chunk(e=e, w=w, out=out):
                            x_j = x_l[edge_index[0, e]]
                            msg = x_j @ w
                            ew = getattr(self, 'edge_weight', None)
                            if ew is not None:
                                msg = msg * ew[e].unsqueeze(-1)
                            return out.index_add_(0, edge_index[1, e], msg)

                        out = checkpoint(_chunk, use_reentrant=False)
                        ew = getattr(self, 'edge_weight', None)
                        deg.index_add_(
                            0, edge_index[1, e],
                            (ew[e] if ew is not None
                             else torch.ones(e.size(0), device=out.device)).to(out.device))
